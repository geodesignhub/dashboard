...
const baseurl = 'https://www.geodesignhub.com/api/v1/projects/';
...
app.get('/images/:diagram_id(\\d+)/:uuid?/', async function (request, response) {
    const reqUUID = request.params.uuid;
    const diagram_id = parseInt(request.params.diagram_id);
    ...
    if (is_uuid && is_int) {
        ...
        response.status(400).send('Invalid parameters supplied.');
    }
...
app.get('/diagrams', async function (request, response) {
    try {
        ...
        const thumbnailData = diagramThumbnailsResponse.data;
        const thumbnailsUuid = thumbnailData['results_url'][0].split('/').slice(-2)[0];

        response.render('diagrams', {
            "status": 1,
            "systems": systemsResponse.data,
            "thumbnails_uuid": thumbnailsUuid,
